...
const listingCreate = async (
    ...
    const Jobcategory = await Category.findById("65acc452a94b559b1e79c447"); // Real Estate //adjust
    ...
    Jobcategory.specific = subCategory;
...
async function startCrawler() {
    const links = [
        "https://www.expatriates.com/classifieds/bahrain/electronics/",
        "https://www.expatriates.com/classifieds/bahrain/computers/",
        "https://www.expatriates.com/classifieds/bahrain/mobile-phones/",
    ];

    for (;;) {
        const randomIndex = Math.floor(Math.random() * links.length);
        const randomLink = links[randomIndex];
        console.log(`Crawling link: ${randomLink}`);
        await crawler.run([randomLink]); // Replace the static link with randomLink
        // Wait for a random time interval before crawling the next link
        const randomDelay = Math.floor(Math.random() * 60000) + 60000; // Random delay between 1 and 2 minutes
        console.log(
            `Waiting for ${
                randomDelay / 1000
            } seconds before crawling the next link...`
        );
